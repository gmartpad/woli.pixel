...
export function BrandConsistencyResults({ score, issues, colorMatches, hasForbiddenColors }: Props) {
  ...
  const scoreBg = score >= 80 ? "bg-success-container" : score >= 50 ? "bg-warning-container" : "bg-error-container";
        ...
        <div className="space-y-2">
          ...
          {colorMatches.map((match, i) => (
            <div key={i} className="flex items-center gap-2 text-xs">
              ...
              <span className={match.within_tolerance ? "text-success" : "text-error"}>
                ΔE {match.distance}
              </span>
              {match.within_tolerance ? (
                <svg className="h-3.5 w-3.5 text-success" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
              ) : (
